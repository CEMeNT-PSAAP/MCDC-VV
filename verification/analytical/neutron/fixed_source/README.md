...
This case occupies $0 \le x \le 10$ cm and has total cross section $\Sigma_{t,g}=1$ cm$^{-1}$ in both groups.
Its scattering matrix is indexed by outgoing then incoming group and every entry is $0.2$ cm$^{-1}$.
The remaining $0.6$ cm$^{-1}$ in each incoming group is capture.

The manufactured total isotropic right-hand sides are
...
For $b_g=R_g'(x)$, the angular solution is

$$
\psi_g(x,\mu)
=R_g(x)-\mu b_g
+\mu b_g
\exp\!\left(-\frac{d(x,\mu)}{|\mu|}\right),
$$

where $d=x$ for $\mu>0$ and $d=10-x$ for $\mu<0$.
This form makes the incoming angular flux isotropic on both vacuum boundaries, allowing MC/DC's white half-space sources to impose it exactly.

The group slopes are equal and opposite, so their exponential-integral boundary terms cancel in the scattering source.
The resulting angle-integrated external volume sources are the positive linear functions
...
The scalar-flux reference uses the exponential integral $E_3$:

$$
\phi_g(x)
=2R_g(x)
+b_g\left[E_3(x)-E_3(10-x)\right].
$$

MC/DC samples each volume source with a piecewise-linear spatial PDF and samples the incident currents with white half-space boundary sources.
The reference implementation evaluates exact mesh-cell averages and normalizes the flux to one total external source particle.
...
- K. Salari and P. Knupp, [*Code Verification by the Method of Manufactured Solutions*](https://www.osti.gov/biblio/759450), SAND2000-1444, Sandia National Laboratories, 2000.
